...
modes.forEach(({ mode }) => {
    test(`${mode}: "Periode"-rules`, async ({ page }) => {
    ...
    test(`${mode}: "Time in relation to now"-rules`, async ({ page }) => {
        await launchAndStart(mode, 'normal')(page)
        ...
        await test.step('TILBAKEDATERT_MER_ENN_3_AR', async () => {
            ...
            const [fom] = await fillPeriodeRelative({ nth: 0, type: '100%', fromRelative: diffSince3Years, days: 14 })(
                page,
            )
     ...
     */
    test.fail(`${mode}: UGYLDIG_KODEVERK_FOR_HOVEDDIAGNOSE, UGYLDIG_KODEVERK_FOR_BIDIAGNOSE`, async () => {
