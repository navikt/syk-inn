...
| Sikkerhet | Autentisering mot HelseID   | EPJ bør benytte HelseID for autentisering og autorisasjon                                                                                                                                                                                                                                                                  | Må   |
...
| Nav       | Eksponert .well-known/smart-configuration endepunkt | Nav må ha tilgang til godt kjente endepunkter (såkalte .well-known filer) for autentisering og riktig bruk av SMART on FHIR. Se [conformance dokumentasjon](https://healthedata1.github.io/smart-app-launch/conformance.html#using-well-known).                                                                                                    | Må   |
...
| --------------------- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| Eksisterende standard | [Påkrevde FHIR ressurser for preutfylling fra EPJ](_oversikt.md) | For preutfylling fra EPJ til sykmeldingsapplikasjonen må følgende FHIR ressurser være definert og tilgjengelige i EPJ:<br>- Pasient (no-basis-Patient)<br>- Helsepersonell (no-basis-Practitioner)<br>- Helsepersonell rolle (no-basis-PractitionerRole)<br>- Organisasjon (no-basis-Organization)<br>- Konsultasjon (Encounter) | Må   |
| Eksisterende standard | [Tilbakeskriving av PDF til journal](_oversikt.md)               | For tilbakeskriving fra sykmeldingsapplikasjonen til EPJ må følgende FHIR ressurser være definert og tilgjengelige for skriving i EPJ:<br>- DocumentReference (DocumentReference)<br>- Binary (Binary)                                                                                                                           | Må   |
| Eksisterende standard | [Tilbakeskriving av strukturerte data](_oversikt.md)             | For tilbakeskriving av FHIR-strukturerte sykmeldingsdata til EPJ må følgende FHIR ressurser være definert og tilgjengelige for skriving i EPJ:<br>- Bundle (Bundle)<br>- QuestionnaireResponse (QuestionnaireResponse)                                                                                                           | Bør  |
| Eksisterende standard | [Batch Bundle for tilbakeskriving](bundle.md)                    | For tilbakeskriving av FHIR-strukturerte sykmeldingsdata bør EPJ sin FHIR-server støtte prosessering av en `batch`-Bundle (POST til base-URL). I en batch behandles hver entry uavhengig, slik at den lovpålagte DocumentReference alltid lagres selv om QuestionnaireResponse skulle feile.                                     | Bør  |
