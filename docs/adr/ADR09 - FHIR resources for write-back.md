# ADR09 - FHIR Resources for Write-Back
...
sequenceDiagram
    ...
    Nav ->> FHIR: POST /QuestionnaireResponse
    Nav ->> FHIR: POST /DocumentReference
```

## Decision

Structured write-back must adhere to the following rules:

1. Structured write-back is an optional benefit for those who want it.
2. Structured write-back must **NOT** be burdensome for Nav or EHR vendors.
...
- Write-back is optional for the EHR; Nav takes no responsibility for missing support and always provides a PDF
...
- Requires 2 FHIR calls per sick note (POST QuestionnaireResponse + POST DocumentReference)
- If POST QuestionnaireResponse succeeds but POST DocumentReference fails, an orphaned QuestionnaireResponse will remain
  on the FHIR server
...
| --- | ----------------------------------------------- | -------------------------------------------------- | --------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `sykmeldingId`                                  | `DocumentReference.id`                             | `{ "resourceType": "DocumentReference", "id": "sykmeldingId" }` | The id of DocumentReference is set to the sick note ID for traceability                                                                                                                                                                                                                         |
...
flowchart TD
    ...
    D -->|404| E[POST /DocumentReference\nPDF only]
    E --> F{Response?}
    F -->|4xx / 5xx| ERR4[log.error\nabort]
    F -->|2xx| G[POST /QuestionnaireResponse]
    G --> H{Response?}
    H -->|4xx / 5xx| ERR5[log.error\nDocumentReference kept without QR link]
    H -->|2xx| I[PUT /DocumentReference/sykmeldingId\nadd context.related QR reference]
    I --> J{Response?}
    J -->|2xx| OK[log.info: success]
    J -->|4xx / 5xx| ERR6[log.error]
```
...
3. Front-end performs `GET /DocumentReference/{sykmeldingId}` as an idempotency check:
    ...
    - `404` → proceed
4. Front-end sends `POST /DocumentReference` with PDF only (id set to sykmeldingId):
    - `4xx`/`5xx` → `log.error` (abort)
    - `2xx` → DocumentReference created, proceed
5. Front-end sends `POST /QuestionnaireResponse` with all sick note items:
    - `4xx`/`5xx` → `log.error` (skip QR reference update, DocumentReference remains without QR link)
    - `2xx` → QR created, retrieve QR id, proceed
6. Front-end sends `PUT /DocumentReference/{sykmeldingId}` to add `context.related` reference to the
...
#### QuestionnaireResponse (standalone resource)
...
#### DocumentReference (with reference to QuestionnaireResponse)
...
| 4   | What happens if the EHR does not support structured write-back?                       | DocumentReference with PDF only is sent as today. Structured data is an added benefit, not a requirement.                                                                                                                                                                     |
...
| 7   | What happens if POST QuestionnaireResponse succeeds but POST DocumentReference fails? | QuestionnaireResponse remains as an "orphan" on the FHIR server. It contains valid clinical data and a reference to the Questionnaire, so it is not harmful. Nav will attempt to write DocumentReference again after a period; if this still fails, the EHR will be notified. |

## Notes

There are a few things that still need to be clarified before this ADR can be considered complete:

- Write-back of medical billing codes (takster)
